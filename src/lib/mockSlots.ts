...
/** Returns a human-readable "next available" label. */
export function getNextAvailableLabel(): string {
  const date = getFirstAvailableDate();
...
export function generateTimeSlotsForDay(
  date: Date,
): { time: string; time24: string; status: SlotStatus }[] {
